...
def build_line(temp_verse, lexicon):
    ...
    while (len(possible_meters) > 0):
        infl = sentence.get_current_sentence().get_next_infl()
        ...
        #print ("Placing ", word)
        sentence.get_current_sentence().place_word(word)
        ...
        if (try_verse is not None):
            ...
            #print ("Removing ", word)
            sentence.get_current_sentence().remove_word()
            continue
    return None
